...
public final class LearningStore {
    ...
    public boolean completed(Episode episode) {
        if (preferences.getBoolean("partial_audio_" + episode.key, false)) return false;
    ...
    public void savePartialAudio(Episode episode, boolean partial) {
        SharedPreferences.Editor editor = preferences.edit().putBoolean("partial_audio_" + episode.key, partial);
        if (partial) editor.putBoolean("completed_" + episode.key, false);
        editor.apply();
    }
    ...
    public void useAlignmentRevision(Episode episode, String revision) {
        String key = "alignment_revision_" + episode.key;
        if (revision.equals(preferences.getString(key, ""))) return;
        // Offsets used to compensate a guessed PDF timeline are invalid for the
        // real audio-derived timeline. Migrate once, preserve later user tuning.
        preferences.edit().putString(key, revision)
                .putLong("subtitle_offset_" + episode.key, 0L).apply();
    }
