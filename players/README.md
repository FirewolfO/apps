...
面向 Android 手机、平板和折叠屏的在线视频应用。界面采用深色双列卡片、精选焦点图、分类筛选和底部导航，播放器支持原画/高清切换、字幕、断点续播、收藏和画中画。
...
APK 位于 `app/build/outputs/apk/debug/app-debug.apk`。应用 ID 为 `com.firewolf.players`，版本为 `1.0.0`（versionCode 1），并从 `https://apps.lxvb.top/api/apps/players/latest` 每日检查新版。
